...
$(document).on("click", "p", function () {
    // Empty the commentss from the comments section
    $("#comments").empty();
    ...
    $.ajax({
        ...
        .then(function (data) {
            ...
            // A textarea to add a new comments body
            $("#comments").append("<textarea id='bodyinput' name='body'></textarea>");
            // A button to submit a new comments, with the id of the article saved to it
            $("#comments").append("<button data-id='" + data._id + "' id='savecomments'>Save comments</button>");

            // If there's a comments in the article
            if (data.comment) {
                // Place the title of the comments in the title input
                $("#titleinput").val(data.comment.title);
                // Place the body of the comments in the body textarea
...
$(document).on("click", "#savecomments", function () {
    ...
    // Also, remove the values entered in the input and textarea for comments entry
    $("#titleinput").val("");
    $("#bodyinput").val("");
});
